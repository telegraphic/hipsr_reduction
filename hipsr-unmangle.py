...
def unmangle(sd_filename):
    ...
    for row in range(sd_len):
        ...
        z = np.append(sd[1].data["DATA"][row,0,0,0],sd[1].data["DATA"][row,0,0,1])
        xpol = z[0::2]
        ypol = z[1::2]

        sd[1].data["DATA"][row,0,0,0] = xpol[::-1]
        sd[1].data["DATA"][row,0,0,1] = ypol[::-1]
...
class Window(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        self.convert_button = self.createButton("&Unmangle", self.convert)
    ...
    def convert(self):
        ...
        for file_in in filelist:
            print "(%02d%%) Unmangling %i of %i"%(float(i)/len(filelist)*100, i, len(filelist))
            print "Input file: %s"%file_in
            unmangle(os.path.join(self.in_combox.currentText(),file_in))
            i += 1
