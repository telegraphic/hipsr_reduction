...
def toSpline(freqs, data):
    ...
    for i in range(len(data)): 
        ...
        tck = interpolate.splrep(freqs[::2][unflagged], data_w[::2][unflagged], s=0)
...
def calibrate(filename_in, filename_out):
    ...
    for i in range(0,13):
        ...
        data_s_a   = toSpline(freqs, data_avg_a)
        data_s_b   = toSpline(freqs, data_avg_b)
...
if __name__ == '__main__':
    ...
    input_dir  = '/home/dprice/data/P669/hipsr/raw/'
    output_dir = '/home/dprice/data/P669/hipsr/calibrated-hipsrcal/'
    ...
    i = 0
    for filename_in in filelist:
        i += 1
        print "Calibrating %s (%i of %i)"%(filename_in, i, len(filelist))    
        filepath_in  = os.path.join(input_dir, filename_in)
        filepath_out = os.path.join(output_dir, filename_in)
