...
Efficiently compute a running median
...
def runningMedian(data, window_size):
    it = iter(data)
    r = RunningMedianObj(islice(it, window_size))
    medians = np.array([r.update(elem, True) for elem in it])
...
if __name__ == '__main__':
    ...
    med = runningMedian(data, window_size)

    midpoint = (window_size - 1) // 2
    median = lambda window: sorted(window)[midpoint]
    target_medians = np.array([median(data[i:i+window_size]) for i in range(1, len(data)-window_size+1)])

    print med[0:10]
    print target_medians[0:10]
